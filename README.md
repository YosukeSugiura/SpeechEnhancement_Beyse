...
　　<img src="https://latex.codecogs.com/gif.latex?\dpi{150}&space;\small&space;p(G|\hat{x},\hat{s})=p(\hat{x},\hat{s},G)p(G)" title="\small p(G|\hat{x},\hat{s})=p(\hat{x},\hat{s},G)p(G)" />
