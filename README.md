...
ここで，
    ...
    - p(s|x) : 事後分布．x が発生したときのその原因が s である確率分布．
    - p(x|s) : 尤度．s を原因として x が発生する確率分布．
...
## MAP推定
 ...
 y の推定に寄与しないことがわかる．したがって，実際の推定問題では p(x) を無視して，
 
    y = argmax_s  p(x|s) p(s)

を解くことになる．
この推定の枠組みを最大事後確率推定（=**MAP推定**）と呼ぶ．
音声強調におけるMAP推定の働きを噛み砕いて説明すると，
「雑音混入音声 x が与えられたとき，取りうる原音声 s の値それぞれに対して，x を生みだす元の音声としてどれだけふさわしいか（=尤度）を計算する．
さらに尤度を s の発生確率で補正し，尤度が最大となる s の値を最終的な出力信号 y をとして推定する．」
ことである．

式からわかるように，**事後分布 p(x|s) と事前分布 p(s) が設定できれば，MAP推定を用いることができる**．
ここで事前分布 p(s) は事前の s に対する知識であるので，使用者が事前に決定しておく必要がある．
...
MAP推定においては，事前分布 p(s) を設定する他に，尤度 p(x|s) を設計する必要がある．
音声強調に置いて尤度 p(x|s) は「原音声 s に起因して 雑音混入音声 x が生成される確率」を表している．
雑音が平均 0 ，分散 σ^2 の正規分布に従い生成されると仮定すると，尤度は次式で表すことができる．

     p(x|s) = a exp(  -| x - s |^2 / 2σ^2  )

ここで a は分散に依存する定数である．この関数は，x = s のときに最大値をとる．
